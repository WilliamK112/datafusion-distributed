...
use crate::common::{
    TreeNodeExt, discover_dynamic_filter_producers, now_ns, task_ctx_with_extension,
};
...
use crate::{
    ...
    SetPlanRequest, TaskCompletedDynamicFilters, TaskKey, WorkUnitFeedDeclaration,
...
impl<'a> StageCoordinator<'a> {
            ...
            while let Some(msg) = worker_to_coordinator_rx.recv().await {
                match msg {
                    ...
                    WorkerToCoordinatorMsg::ProducedDynamicFilter(filter) => {
                        dynamic_filter_registry.add_partial(task_key, *filter);
                    }
                }
            }
    ...
    fn task_specialized_plan(&self, task_i: usize) -> Result<SpecializedTaskPlan> {
        ...
        let dynamic_filter_remote_producer_ids =
            dynamic_filter_remote_producer_ids(&transformed.data)?;
