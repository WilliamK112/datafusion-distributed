...
pub(crate) async fn execute_range_partitioned_query(
    ...
    let ctx = ctx
        ...
        .with_distributed_desired_task_count_handler(1usize)
        .with_distributed_cardinality_effect_task_scale_factor(0.5)?;
    {
        let state = ctx.state_ref();
        let mut state = state.write();
        let options = state.config_mut().options_mut();
        options.execution.target_partitions = 4;
        options.optimizer.hash_join_single_partition_threshold = 0;
        options.optimizer.hash_join_single_partition_threshold_rows = 0;
    }
    register_range_partitioned_table(&ctx, "range_dim", "testdata/join/parquet/dim", "d_dkey")
        .await?;
    register_range_partitioned_table(&ctx, "range_fact", "testdata/join/parquet/fact", "f_dkey")
        .await?;
...
async fn register_range_partitioned_table(
    ...
    let output_partitioning = Partitioning::Range(RangePartitioning::try_new(
        ...
        ["B", "C", "D"]
            .into_iter()
            .map(|value| SplitPoint::new(vec![ScalarValue::Utf8(Some(value.to_string()))]))
            .collect(),
    )?);
    ...
    let table_path = format!("{}/{relative_path}/", env!("CARGO_MANIFEST_DIR"));
    let config =
        ListingTableConfig::new(ListingTableUrl::parse(table_path)?).with_listing_options(options);
    let state = ctx.state();
